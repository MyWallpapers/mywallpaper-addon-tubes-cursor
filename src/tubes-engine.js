...
export function TubesCursor(canvas, opts = {}) {
  ...
  function updatePointerFromLayer(x, y, width, height) {
    if (
      !Number.isFinite(x)
      || !Number.isFinite(y)
  ...
  function onMouseMove(e) {
    updatePointerFromLayer(e.clientX - rectLeft, e.clientY - rectTop, rectW, rectH);
  }

  function onHostMessage(event) {
    const message = event.data;
    if (!message || message.source !== 'mywallpaper') return;
    if (message.type === 'MYWALLPAPER_POINTER_MOVE') {
      updatePointerFromLayer(
        Number(message.x),
        Number(message.y),
    ...
    if (message.type === 'MYWALLPAPER_POINTER_LEAVE') {
      mouseOver = false;
    }
  }
  ...
  function animate() {
    ...
    const delta = Math.min(clock.getDelta(), 1 / 30); // Clamp to 30fps min to prevent trail collapse on GC/throttle spikes.
